...
io.on('connection', (client) => {
	...
	function handleKeydown(keyCode) {
		...
		if (vel && state.length !== 0) {
			// Update velocity of players based on current player
			try {
				if (
					state[roomName].players[client.number - 1].vel.x === 1 &&
					vel.x === -1
				) {
					//pass
				} else if (
					...
					state[roomName].players[client.number - 1].vel.y === 1 &&
					vel.y === -1
				) {
					//pass
				} else if (
...
function startGameInterval(roomName) {
	const intervalId = setInterval(() => {
		...
		if (!winner) {
			// if there is no winner we continue to send the game state to the client
			emitGameState(roomName, state[roomName]);
		} else {
			// if we get a winner we emit a gameOver event to client
